...
public class Tokenizer<T> {
    ...
    public MyNumber<T> parser;


    public Tokenizer(String string, MyNumber<T> parser) {
        this.parser = parser;
    ...
    private void NumberToken(char c) throws ArithmeticException{
        ...
        currentIndex--;
        try {
            lastValue = parser.parseToDigit(sb.toString());
    ...
    private void expect(String s) throws ParserMyException {
        ...
        if (!s.equals(sb.toString())) {
            throw new ParserMyException("WRONG_OPERATION_ERROR: Unary expression.operation has mistake| Expression: " + getExpression(), currentIndex, expression.length());
        }
    }
}